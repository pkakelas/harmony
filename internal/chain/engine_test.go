...
func TestApplySlashing(t *testing.T) {
	...
	// Inital Reporter balance: 0
	// Initial Validators self-delegation: FourtyKOnes

	if err := applySlashes(chain, state, slashes); err != nil {
		t.Error(err)
	}

	expDelAmountAfterSlash := twentyKOnes
	expRewardToReporter := tenKOnes
	...
	reporterBalanceAfterSlash := state.GetBalance(reporterAddr)
	if reporterBalanceAfterSlash.Cmp(expRewardToReporter) != 0 {
		t.Errorf("Slashing reward was not added properly to reporter: %v/%v", expRewardToReporter, reporterBalanceAfterSlash)
	}
}
